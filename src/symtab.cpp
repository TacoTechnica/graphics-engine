...
void print_light(Light *p)
{
    Vector3f *dir = p->getDirection();
    Vector3f *col = p->getDirection();
  printf("Direction (Position) -\t %6.2f %6.2f %6.2f\n",
        dir->getX(), dir->getY(), dir->getZ());
        //p->l[0],p->l[1],p->l[2]);

  printf("Color (Brightness) -\t r:%6.2f g:%6.2f b:%6.2f\n",
...
SYMTAB *add_symbol(char *name, int type, void *data)
{
  ...
  switch (type)
    ...
    case SYM_CONSTANTS:
      ...
      break;
    case SYM_MATRIX:
      t->s.m = (Matrix *)data;//(struct matrix *)data;
      break;
    case SYM_LIGHT:
      t->s.l = (Light *)data;//(struct light *)data;
      break;
    case SYM_VALUE:
